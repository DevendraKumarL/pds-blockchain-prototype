...
window.stateApp = {
    ...
    showRegister: function() {
        var self = this;
        self.hideAll();
        var registerDiv = document.getElementById('state-goverment-register');
        registerDiv.style.display = "block";
    },

    registerStateGovernment: function() {
        var self = this;
        var name = document.getElementById("s-g-name");
        var email = document.getElementById("s-g-email");
        var pass = document.getElementById("s-g-password");
        ...
        userGlobal.addUser(stateGovernmentAddress, name.value, email.value, 0, pass.value, uplace, {from: centralGovernmentAddress, gas: 200000}).then(function(res){
            console.log(res);
            userDb[stateGovernmentAddress] = true;
            name.value = "";
            email.value = "";
    ...
    showLogin: function() {
        var self = this;
        self.hideAll();
        var loginDiv = document.getElementById("state-goverment-login");
    ...
    login: function() {
        ...
        var pass = document.getElementById('s-g-password-login');
        if ($('input:radio[id="radio1"]').is(':checked')) {
            var addr = document.getElementById('s-g-address-login');
            ...
            .then(function(res) {
                ...
                alert("stateGovernment not Authenticated");
                return;
            });
        }
        else if ($('input:radio[id="radio2"]').is(':checked')) {
            var email = document.getElementById('s-g-email-login');
    ...
    storeLoginSessionCookie: function(userId, expDays, userinfo) {
        ...
        document.cookie = "state=" + userDetails + ";expires=" + expires + ";path=/";
